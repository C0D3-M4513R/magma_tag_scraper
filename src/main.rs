...
async fn run() -> Result<(),()> {
    simple_logger::SimpleLogger::new()
        ...
        .init()
        .expect("Failed to initialize logger. Setting the logger for the first time should not fail.");
    ...
    while let Some(future) = js.join_next().await {
        if let Ok((version, res)) = future {
            match res {
                ...
                Err(e) => log::error!("Failed to get libs for version {}: {}",version.to_string(),e),
            }
        }
    }
    return Ok(());
...
async fn get_lib_list(version:Version) -> (Version,Result<(), Error>) {
    ...
    log::info!("Got body for version {}. Parsing JSON.",version.to_string());
    ...
    match folder_r {
        Err(e) => return (version,Err(Box::new(e))),
        Ok(e) => folder = e.map(|e|e.ok()).filter(Option::is_some).map(|e|unsafe{e.unwrap_unchecked()}).collect(),
    }
        ...
        Ok(mut versions) => {
            ...
            for i in &folder{
                if let Some(file_name) = i.file_name().to_str(){
                    if versions_old.iter()
                        .filter(|e|get_name(e.get_link()) == file_name || get_name(e.get_installer_link()) == file_name)
                        .count() > 0 {
...
fn download_link(folder:&Vec<DirEntry>, folder_path:&PathBuf, link:&String, js:&mut JoinSet<Result<(), Error>>) {
    ...
    }else { log::info!("{} already exists",path.display()) }
}
...
async fn remove_version(i:PathBuf) -> Result<(), Error> {
    match tokio::fs::remove_file(&i).await {
        ...
        Err(e) => Err(Box::new(std::io::Error::new(ErrorKind::Other, format!("Failed to delete {}: {}", i.display(), e)))),
    }
}
