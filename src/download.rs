...
static DL_LOCKS: [OnceCell<tokio::sync::Mutex<()>>;5]  = [OnceCell::const_new(), OnceCell::const_new(), OnceCell::const_new(), OnceCell::const_new(), OnceCell::const_new()];
const NOT_FOUND:bytes::Bytes = bytes::Bytes::from_static(br#"{"message":"404 Project Not Found"}"#);
...
pub async fn fetch_url(url: String, file_name: impl AsRef<Path>) -> Result<()> {
	...
	log::info!("Finished Downloading {} to {}",url, file_name.as_ref().display());
	Ok(())
}
...
async fn try_get_bytes(url: impl reqwest::IntoUrl)->Result<bytes::Bytes>{
	...
	if status != reqwest::StatusCode::OK {
		return Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "request was not status code OK")))
	}
	Ok(bytes)
}
